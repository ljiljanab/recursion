...
function greedyMakeChange(target, coins = [25, 10, 5, 1]) {
  ...
  function makeBetterChange(target, coins = [25, 10, 5, 1]) {
   ...
   for (let i = 0; i < coins.length; i++) {
    ...
    if (remainderChange !== null ) {
      let currentChange = [coin, ...remainderChange];
  
      if (betterChange === null || currentChange.length < betterChange.length ) {
        betterChange = currentChange;
      }
    }
   }
   return betterChange;
  ...
  console.log(greedyMakeChange(24, [10, 7, 7]));